...
使用方式::
    ...
    from autowsgr.infra.base.ui.pages.mission_page import MissionPage, MissionPanel
...
from autowsgr.infra.base.ui.pages.mission_page.recognition import (
    find_button_rows,
    recognize_row,
)
...
if TYPE_CHECKING:
    ...
    from autowsgr.infra.base.ui.pages.mission_page.data import MissionInfo
...
class MissionPage:
    ...
    def _try_confirm(self, *, timeout: float = 5.0) -> bool:
        ...
        领取后弹窗可能延迟出现, 轮询等待 :data:`Templates.Confirm` 模板,
        命中即点击按钮中心关闭弹窗; 超时返回 ``False`` (未弹窗或已关闭)。
        """
    ...
    def collect_rewards(self) -> bool:
        ...
        if detail is not None:
            self._ctrl.click(*detail.center)
            return self._try_confirm(timeout=5.0)
    ...
    def recognize_missions(self, screen: np.ndarray) -> list[MissionInfo]:
        ...
        list[MissionInfo]
            可见任务列表, 按从上到下排列。
        """
        from autowsgr.infra.base.ui.pages.mission_page.data import get_all_mission_names
